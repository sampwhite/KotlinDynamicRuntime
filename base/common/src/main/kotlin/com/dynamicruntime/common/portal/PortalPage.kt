...
/**
 * The portal's single HTML page, held as a Kotlin string rather than a bundled
 * resource file (the house style: what would elsewhere be a static asset is a
 * Kotlin declaration). It is a self-contained page -- inline CSS and vanilla JS,
 * no build step, no external assets -- that fetches the endpoint catalog from
 * [PTL.catalog] and renders a form per endpoint.
 *
 * Written as a multi-dollar (`$$`) string so a bare `$` is literal; the one
 * server-side value injected is the catalog path via `$${PTL.catalog}`.
 */
object PortalPage {
    val html: String = $$"""
<!doctype html>
...
<script>
"use strict";
// The platform's endpoint-introspection endpoint (SchemaService) for the canonical list, plus the
// portal's own feed of resolved form fields (the $refs in each endpoint's raw input schema resolved
// into concrete fields, which the browser can't do itself). Merged by collationKey (path:method).
var ENDPOINTS_API = '$${PTL.endpointsApi}';
var FIELDS_FEED = '$${PTL.fieldsFeed}';
