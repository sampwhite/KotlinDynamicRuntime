...
object PTL {
     ...
     * to list the instance's endpoints. A context-root-relative path: the page prefixes it with the API root.
     */
     ...
     * `$ref`s in an endpoint's raw input schema resolved into concrete fields, which the browser can't do. A
     * context-root-relative application path (matched against `appPath`; the page prefixes the content root).
     */
...
class PortalService : ServiceInitializer, ContentServer {
    ...
    /**
     * Serves the portal page and its fields feed, or redirects the content root to the page; passes on
     * anything it does not own. Only engages with content-focused requests, matching on the context-root-
     * stripped [RequestHandler.appPath] (so the configured content root, e.g. `cp`, is transparent here).
     */
