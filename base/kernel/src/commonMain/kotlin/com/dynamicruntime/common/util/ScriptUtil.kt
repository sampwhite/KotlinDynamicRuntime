 ...
 *
 * **A cycle is detected by ancestry, not by a visited set.** The guard is [ScriptState.includeChain], the keys
 * currently being evaluated: a key already in its own chain is a genuine cycle and is named as one, path and
 * all. A *set* of everything seen would be wrong -- one fragment legitimately pulled from two different
 * branches is reuse, not recursion -- and it would refuse a correct template.
 *
 * The depth cap remains as a second, cruder guard for a chain that is acyclic but unbounded: distinct keys
 * generated by a computed expression never repeat, so ancestry alone would not stop them. The two report
 * differently, because "a fragment pulls itself" and "this nests too far" call for different fixes.
 *
 * Errors from inside the fragment are re-reported naming the fragment ([mkFragmentContext]), because their
 * position is an offset into the *fragment's* text and would otherwise point at a place in the calling
