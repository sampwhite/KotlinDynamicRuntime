...
| `KDR_GEDRA_CONFIG_CHECK` | What a Gedra config problem at startup does — a duplicated trait id, a namespace claimed by two owners: `strict` refuses the boot, `warn` logs and carries on with the rest of the instance, `off` skips the check. Unset means **strict everywhere except `prod`, where it warns** — the same paradigm as `KDR_FRAGMENT_CHECK`: a configuration defect degrades in production and refuses everywhere else, because silence while the author is still at the keyboard is how a defect reaches production, while refusing to boot a production node over one bad trait takes down every endpoint that had nothing to do with it. Keyed on the **environment**, never on `isTestInstance` — that flag is inferred from in-memory-ness, so an ordinary local run against a real database would otherwise get production behavior. | strict, except `warn` in `prod` |
