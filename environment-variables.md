...
| `KDR_TRUST_ENV_AUTH_HEADER` | Whether this node believes the `X-Kdr-Env-Email` header an edge server sets on a request it has already authenticated (issue #348). A node **cannot verify** that claim — anyone who can reach its port could set the header, and the real guarantee is that only the edge can reach it, which is a property of the network rather than anything the application can check. So this is the deployment *asserting* it, and a node that is not behind an edge leaves it off and cannot be fooled by the header at all. The `trustEnvAuthHeader` config option wins over this. Note what the header buys is deliberately narrow: env auth is a property of the **channel**, not the user, so it grants no role and never stands in for a login. | on for a test instance, off otherwise |
| `KDR_ASSUME_ENV_AUTH` | Whether this node **invents** env auth for a request that arrived with no `X-Forwarded-For` — the convenience that makes a developer's own box behave like one sitting behind an edge, so the env-authed surface is there from the first page load (issue #360). The identity it carries is deliberately synthetic (`assumed@local.invalid`, in the RFC 2606 never-resolvable TLD, so it can never collide with a real address); a developer who wants a *particular* address has the `forTestingOnly` `/fixture/envAuth` endpoint instead. Independent of `KDR_TRUST_ENV_AUTH_HEADER` on purpose — believing a claim and making one up are different questions. The missing `X-Forwarded-For` is **not** what makes this safe; the test-instance fence is (see `KDR_TEST_INSTANCE`). It separates "through the edge" from "straight at the server" on a machine where a developer does both. The `assumeEnvAuth` config option wins over this. | on for a test instance in `local` **only** — not `unit`, where every request looks direct and the whole test suite would turn env-authed |
| `KDR_COOKIE_SECURE` | Whether the `Set-Cookie`s this node issues carry the `Secure` attribute — session cookies (`kdrAuth`, and the edge's `kdrEnvAuth`) included (issue #431). Set explicitly here rather than derived from `X-Forwarded-For`. In a real deployment the edge (or load balancer) in front of this node sets that header, so it is not directly attacker-supplied; the point is narrower — a cookie's `Secure` flag should be a property of how the node is *served*, not read off a request header at all (hardening, and the kind of line flagged on sight regardless of reachability). The `cookieSecure` config option wins over this. Unset, it defaults from the environment — `Secure` everywhere except `local`/`unit`, so a plain-HTTP `localhost` (the developer's box and the real-socket tests) keeps getting a cookie the browser will return. A deployment terminating TLS in front of a plain-HTTP node still wants this **on**. | secure everywhere but `local`/`unit` |
| `KDR_EDGE_UPSTREAM` | Where an edge server forwards application traffic — every request whose leading path segment is one of the *application's* context roots (`kda`, `cp`, `wa`, `st`) rather than one of the edge's own (`ea`, `ec`, `ew`, `es`), which it serves itself (issue #419). Read only by a node booted in the `edge` role; an ordinary node ignores it. One upstream, which is the first forwarding slice and not the end state — a route table backed by a registry of live nodes replaces it. The `edgeUpstream` config option wins over this. | `http://localhost:7070`, the ordinary development application |
...
| `KDR_ALLOW_SCHEMA_DRIFT` | Boot despite **blocking** schema drift — a column the database has, the code does not declare, that is `NOT NULL` with no default. Startup normally refuses, because the framework cannot populate such a column and so every insert into that table already fails. Setting this true downgrades the refusal to a logged error; it does **not** make writes work. For an operator part-way through a migration. An **allow-flag** rather than a mode word, so it has no `off`; what it resolved to is reported by `GET /operator/boot/checks`. | `false` |
| `KDR_FRAGMENT_CHECK` | What a malformed Markdown fragment does at startup: `strict` refuses the boot, `warn` logs the problems and serves anyway, `off` skips the check. Unset means **strict everywhere except `prod`**, where it is `warn` — a developer or a test should be stopped by broken copy, while a production node should not refuse every unrelated endpoint over one malformed message (the render path already falls back for that one). Deliberately keyed on the environment rather than `isTestInstance`, which is inferred from in-memory-ness and so is false for an ordinary local run. Check a running node with `GET /operator/fragments/check`, and see what this resolved to with `GET /operator/boot/checks`. | unset (`strict`, or `warn` in `prod`) |
| `KDR_GEDRA_CONFIG_CHECK` | What a Gedra config problem at startup does — a duplicated trait id, a namespace claimed by two owners: `strict` refuses the boot, `warn` logs and carries on with the rest of the instance, `off` skips the check. Unset means **strict everywhere except `prod`, where it warns** — the same paradigm as `KDR_FRAGMENT_CHECK`: a configuration defect degrades in production and refuses everywhere else, because silence while the author is still at the keyboard is how a defect reaches production, while refusing to boot a production node over one bad trait takes down every endpoint that had nothing to do with it. Keyed on the **environment**, never on `isTestInstance` — that flag is inferred from in-memory-ness, so an ordinary local run against a real database would otherwise get production behavior. Reported by `GET /operator/boot/checks` like the other boot checks. | strict, except `warn` in `prod` |
