...
| `KDR_COOKIE_SECURE` | Whether the `Set-Cookie`s this node issues carry the `Secure` attribute — session cookies (`kdrAuth`, and the edge's `kdrEnvAuth`) included (issue #431). Set explicitly here rather than inferred from `X-Forwarded-For`, which the caller controls: a security attribute of a cookie *we* issue must not depend on a header a client can omit. The `cookieSecure` config option wins over this. Unset, it defaults from the environment — `Secure` everywhere except `local`/`unit`, so a plain-HTTP `localhost` (the developer's box and the real-socket tests) keeps getting a cookie the browser will return. A deployment terminating TLS in front of a plain-HTTP node still wants this **on**. | secure everywhere but `local`/`unit` |
